...
# Robust checking in database for date, download only from last entry till specified date/ today date when 'last' is passed
# DO NOT run 'date last' twice in a row. If so, use SQL to drop duplicates, code below in ''' '''
# Multi threaded but safe insertion into SQL db + concurrent download from yfin
# Batch insert
